...
(using the C API) be set with C<quvi_setopt(3)>.
 ...
 $opts->{category}        = WWW::Quvi::ProtoAll;    # Default: ProtoHttp
...
libquvi supports these for historical reasons only. Each
video would have one or more video segments which had to be
downloaded separately from different URLs.
...
E<lt>http://repo.or.cz/w/www-quvi.gitE<gt>
